-- Supabase Consolidated Schema Summary
-- Run this script in the Supabase SQL Editor to initialize or update your database schema.
-- This file contains the up-to-date schema, including all recent constraints and indexes.

begin;

-- ---------------------------------------------------------------------------
-- 1) Weekly class schedule table
-- ---------------------------------------------------------------------------
...
-- Setup read-only policies for authenticated users
do $$
declare
    t text;
begin
    for t in select unnest(array[
        'schedules', 'appointments', 'class_sessions', 'exams',
        'elearning_progress', 'elearning_deadlines', 'calendar_sync_state', 'notification_log'
    ]) loop
        execute format('
            drop policy if exists authenticated_read_%I on public.%I;
